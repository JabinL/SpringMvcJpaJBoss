...
app.service('companySharedServices', function () {
    this.listDeptEmps = function ($scope, $http, deptId) {
        $http.get('/SpringMvcJpaJBoss/rest/depts/' + deptId + '/emps').then
        (
            function (response) {
...
app.controller('deptsEmpsMainController', function ($scope, $http, factoryData, companySharedServices) {
    /* Returns All Deps */
    $http.get("/SpringMvcJpaJBoss/rest/depts/").then(
        function (response) {
    ...
    /* Returns All Emps */
    $http.get('/SpringMvcJpaJBoss/rest/emps').then(
        function (response) {
            $scope.emps = response.data;
        }
    )
    ...
    $scope.deleteEMP = function (empId) {
        $http.delete('/SpringMvcJpaJBoss/rest/emps/' + empId).then(
            function (response) {
    ...
    $scope.submitForm = function () {
        ...
        $http.post('/SpringMvcJpaJBoss/rest/depts/' + $scope.emp.deptId + '/emps', $scope.emp).then(
            function (response) {
